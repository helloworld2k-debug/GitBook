...
insert into public.policy_pages (slug, title, summary, body, sort_order)
values
  (
    'terms',
    'Terms of Service',
    'Rules for using GitBook AI downloads, accounts, license codes, desktop authorization, and cloud sync access.',
...
You may contact support to ask about your account information, cloud sync entitlement status, license redemption records, or support history. We may need to verify account ownership before making changes.',
  ...
  (
    'refund',
    'Refund Policy',
    'How direct payments, partner license codes, activation issues, and cloud sync entitlement refunds are reviewed.',
...
drop policy if exists policy_pages_admin_all on public.policy_pages;
create policy policy_pages_admin_all
  on public.policy_pages
  for all
