-- Repair cloud sync entitlement summaries for Dodo grants and make the grant RPC
-- self-heal if a grant ledger row exists but the user-facing summary row is missing.
...
begin
  ...
  if existing_valid_until is not null then
    ...
    do update set
      valid_until = case
        when public.license_entitlements.valid_until >= excluded.valid_until
          then public.license_entitlements.valid_until
        else excluded.valid_until
      end,
      status = case
        when public.license_entitlements.valid_until >= excluded.valid_until
          then public.license_entitlements.status
        else 'active'
      end,
      source_donation_id = case
        when public.license_entitlements.valid_until >= excluded.valid_until
          then public.license_entitlements.source_donation_id
        else excluded.source_donation_id
      end,
      updated_at = now();
...
select distinct on (grant_row.user_id, grant_row.feature_code)
  grant_row.user_id,
  grant_row.feature_code,
  grant_row.valid_until,
  'active',
  grant_row.source_donation_id
from public.license_entitlement_grants grant_row
join public.donations donation_row on donation_row.id = grant_row.source_donation_id
where donation_row.provider = 'dodo'
  and donation_row.status = 'paid'
  and grant_row.feature_code = 'cloud_sync'
order by grant_row.user_id, grant_row.feature_code, grant_row.valid_until desc
on conflict (user_id, feature_code)
do update set
  valid_until = case
    when public.license_entitlements.valid_until >= excluded.valid_until
