...
function isPaginatedUsersResult(value: unknown): value is PaginatedUsersResult {
  return (
    typeof value === "object" &&
    value !== null &&
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: paginatedData, error } = await supabase.rpc("get_admin_users_paginated", {
      input_page: 1,
      input_per_page: 10000, // Large limit for export
      input_search: query,
      input_role_filter: roleFilter,
      input_status_filter: statusFilter,
      input_type_filter: typeFilter,
    ...
    const paginatedResult = paginatedData?.[0];
    if (!isPaginatedUsersResult(paginatedResult)) {
